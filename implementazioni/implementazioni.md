...
10. togli il principio zero-dipendenze, migliora il progetto in base a questo cambiamento, riporta le idee in questo file (implementazioni.md), elenca anche tutte le dipendenze che si potrebbero installare e la descrizione di ognuna
    ...
    **FATTO (07/09/2026)** — branch `task/via-lo-zero-dipendenze`. Il principio è tolto:
    ...
    La regola in breve, il prezzo che si paga e **il registro di quello che c'è installato**
    stanno in [`documentazione/dipendenze.md`](../documentazione/dipendenze.md), che è il
    posto da guardare prima di installare qualcosa. Qui sotto c'è il ragionamento per
    esteso e l'elenco completo di cosa si potrebbe installare.
    ...
    1. **Si sblocca l'uscita sugli store**, cioè il punto 32 di
       [`08-uscita-sugli-store.md`](08-uscita-sugli-store.md). Electron, Capacitor e le API
    ...
    5. **Le licenze diventano un problema vero.** In un gioco _venduto_ una GPL/AGPL non ci
    ...
    - `npm audit` dentro `npm run verifica` — **fatto**, è l'ultimo passo della verifica del
      frontend — e **Dependabot o Renovate** acceso sul repo;
    - dove si può, si installa con `--ignore-scripts`;
    - un **registro delle dipendenze** — **fatto**, è
      [`documentazione/dipendenze.md`](../documentazione/dipendenze.md): pacchetto, a cosa
    ...
    1. **Il guscio nativo** (Electron + electron-builder, Capacitor, steamworks.js).
       È il punto 32 di [`08-uscita-sugli-store.md`](08-uscita-sugli-store.md), e senza
       dipendenze non parte proprio.
    ...
    6. **I salvataggi**, cioè il punto 34 di
       [`08-uscita-sugli-store.md`](08-uscita-sugli-store.md): `idb-keyval` per uscire dal
       `localStorage` e `fflate` per comprimere prima di mandarli al server, dove il tetto
       è 2 MB.
    ...
    - **esbuild** — _già installata._ Mette insieme e minifica i 13 CSS e i 72 JS in due
    ...
    - **vitest** — il runner di prove: guarda i file e rilancia da solo, dice cosa non è
      ...
      logica del gioco senza aprire Chrome. _(Alternativa senza installare niente:
      `node:test`, che Node ha già dentro — meno comodo, zero pacchetti.)_
    ...
    - **@playwright/test** — Chrome, Firefox e Safari veri, senza finestra: apre il gioco,
      ...
      `prova-sul-telefono`, ma automatico e dentro alla CI. _Costo: si scarica qualche
      centinaio di MB di browser._
    ...
    - **sharp** — il coltellino delle immagini: ridimensiona, converte in WebP/AVIF,
      ...
      pacchetto più di qualunque altra cosa. _Costo: è codice nativo (libvips), si scarica
      un binario all'installazione._
    ...
    - **husky** + **lint-staged** — fanno girare linter e prove **prima** del commit, e
      solo sui file toccati. _(In `.githooks/` c'è già qualcosa: forse basta quello.)_
    ...
    - **idb-keyval** — `localStorage` ma su IndexedDB: asincrono, senza il tetto dei 5 MB,
      e non sparisce quando il sistema fa pulizia. È il punto 34 di
      [`08-uscita-sugli-store.md`](08-uscita-sugli-store.md). ~1 KB.
    ...
    - **motion** (Motion One) — animazioni e transizioni appoggiate a quelle del browser,
      ~5 KB. È l'aggancio naturale del punto sulle transizioni fra studio, sala e casa, qui
      sopra nella lista DA FARE.
    ...
    - **uplot** — grafici piccoli e velocissimi (~45 KB): è l'app «come stanno andando le
      canzoni nel tempo» del punto sulla TRACK, qui sopra.
    ...
    - **pg** — _già installata._ Il client PostgreSQL: SCRAM-SHA-256, TLS, tipi,
    ...
    - **node-cron** — _non serve:_ il giro di settimana parte da sé alla prima richiesta
    ...
    - **`npm audit`** dentro `npm run verifica` — è già dentro npm, e adesso è dentro anche
      alla verifica.
    ...
    [`00-come-si-lavora.md`](00-come-si-lavora.md): dove «zero dipendenze» era una _regola_
    adesso c'è la regola nuova; dove era la _descrizione di com'è fatto oggi_ è rimasta,
    corretta (il backend una dipendenza ce l'ha, `pg`, e il gioco ha esbuild).
