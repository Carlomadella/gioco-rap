...
const cronometra = async (cosa, quante, f) => {
  const via = process.hrtime.bigint();
  let ultimo;
  for(let i = 0; i < quante; i++) ultimo = await f(i);
...
async function misura(){
  ...
  const via = Date.now();
  const A = await archivio.apri({ file: FILE, quantiBot: 0, settimanaMs: 24 * 3600e3 });
  ...
  while(messi < QUANTI){
    ...
    await A.insieme(async () => {
      for(const b of popolazione(blocco, usati)){
        ...
        await A.fai(`INSERT INTO artista (id, bot, nome, citta, genere, storia, seed, stream, uscite, deal,
          ...
          eBot ? null : Date.now());
        if(eBot) await A.fai("INSERT INTO bot_stato (artista_id, slancio, caldo, carattere) VALUES (?,0,0,?)",
          b.id, b.carattere);
      }
    });
    messi += blocco;
  ...
  const acc = await archivio.creaAccount({ tipo: "ospite", idEsterno: "carico" });
  const io = await archivio.iscriviArtista({ accountId: acc.id, nome: "Quello Della Prova",
    citta: "Rovereto", genere: "trap", seed: 1 });
  await archivio.segnaPunteggio(io.id, { stream: 40000, fan: 2500, livello: 8, uscite: 4 });

  console.log("le cose che il server fa a ogni richiesta:");
  await cronometra("la top 10", 20, () => archivio.classifica(1, 10, null));
  await cronometra("la top 100", 20, () => archivio.classifica(1, 100, null));
  await cronometra("la top 100 con la mia riga", 20, () => archivio.classifica(1, 100, io.id));
  await cronometra("la fetta 5000-5100", 20, () => archivio.classifica(5000, 100, null));
  await cronometra("chi ho davanti e dietro", 20, () => archivio.intorno(io.id, 4));
  await cronometra("la classifica della mia città", 20, () => archivio.classifica(1, 50, io.id, { citta: "Rovereto" }));
  await cronometra("un punteggio (con i traguardi)", 10, i =>
    archivio.segnaPunteggio(io.id, { stream: 40000 + i * 100, fan: 2500, livello: 8, uscite: 4 }));
  await cronometra("il feed del telefono", 10, () => archivio.feed(io.id, 20));
  await cronometra("gli opps", 10, () => archivio.opps(io.id, 3));

  console.log("\nle cose che fa una volta ogni tanto:");
  await cronometra("un giro di settimana intero", 1, () => archivio.giroSettimana());
  await cronometra("un secondo giro", 1, () => archivio.giroSettimana());

  const stato = await archivio.stato();
