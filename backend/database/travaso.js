...
const A = attrezzi(apriDb(A_FILE));
if(A.uno("SELECT id FROM artista LIMIT 1")){
...
A.insieme(() => {
  ...
  if(!A.uno("SELECT id FROM stagione LIMIT 1"))
    A.fai("INSERT INTO stagione (nome, inizio, stato) VALUES ('Prima stagione', ?, 'corrente')", vecchio.creato || ora);
  const stagione = A.uno("SELECT id FROM stagione ORDER BY id DESC LIMIT 1").id;
  const fino = Math.max(1, Number(vecchio.settimana) || 1);
  for(let n = 1; n <= fino; n++){
    A.fai("INSERT OR IGNORE INTO settimana (numero, stagione_id, iniziata, chiusa) VALUES (?,?,?,?)",
      n, stagione, vecchio.creato || ora, n < fino ? (vecchio.aggiornato || ora) : null);
  }
  A.fai("INSERT INTO stato (chiave, valore) VALUES ('prossimo_giro', ?) " +
  ...
  for(const v of artisti){
    ...
    if(v.bot){
      A.fai("INSERT INTO bot_stato (artista_id, slancio, caldo, carattere) VALUES (?,?,?, 'normale')",
        id, v.mom || 0, v.hot || 0);
      conto.bot++;
    } else {
      /* un account da ospite per ognuno, con l'artista attaccato */
      const acc = uuid();
      A.fai("INSERT INTO account (id, lingua, creato, visto) VALUES (?, 'it', ?, ?)",
        acc, v.creato || ora, v.ultimo || ora);
      A.fai("INSERT INTO identita (id, account_id, tipo, id_esterno, creato) VALUES (?,?,'ospite',?,?)",
        uuid(), acc, uuid(), v.creato || ora);
      A.fai("UPDATE artista SET account_id = ? WHERE id = ?", acc, id);
    ...
    if(v.posPrec && fino > 1){
      A.fai("INSERT OR IGNORE INTO classifica_posizione (settimana, artista_id, pos, stream, delta) VALUES (?,?,?,?,NULL)",
