...
| Source Separation pilot | **RECEIPT PREPARATO / INFERENZA NON ANCORA AVVIATA** | `source-separation-development-inference-v1-001` creato append-only con `AUTHORIZED_NO_INFERENCE`, 8/8 record e source family bloccate; preparazione receipt ha rieseguito il pre-inference gate senza aprire audio, eseguire separazione o accedere al final holdout; prossimo gate = check read-only del receipt congelato prima del batch |
