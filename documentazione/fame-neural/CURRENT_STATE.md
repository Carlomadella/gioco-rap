...
| Source Separation pilot | **PRE-INFERENCE PASS / RECEIPT DA PREPARARE** | run 8/8 development già preparato; environment lock, prepared-run check, adapter self-test e signature OpenVINO reale PASS su CPU; nessun audio/inferenza/holdout access nel gate; adapter + review rubric + execution contract + batch runner append-only implementati; prossimo gate = creare e verificare il receipt `source-separation-development-inference-v1-001` prima del batch |
