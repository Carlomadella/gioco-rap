...
Source Separation pilot **preparato**: preflight reale passato su 8/8 family development e run append-only `source-separation-pilot-v1-001` creato con SHA delle sorgenti verificati; inferenza non ancora avviata. Baseline primaria: `intel-openvino-htdemucs-v4-97fc578`. Doctor reale PASS: Audacity 3.7.1, `mod-openvino.dll`, `mod-script-pipe.dll` e i due hash HTDemucs congelati sono presenti. La pipe reale è stata aperta con successo su Windows; il primo discovery ha però filtrato genericamente tutti gli effetti OpenVINO. L'helper è stato ristretto a `OpenVINO Music Separation` e ora verifica anche se il comando espone parametri automatizzabili prima di autorizzare il batch adapter. Prossimo intervento: abilitare `mod-script-pipe`, riavviare Audacity, eseguire discovery → batch adapter → QA stems. Training serio resta chiuso.
