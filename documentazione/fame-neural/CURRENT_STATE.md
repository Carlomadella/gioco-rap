...
| Trascrizione Audio→MIDI pilot | **BLOCCO APERTO / PROTOCOLLO CONGELATO / BASELINE IMPLEMENTATA** | protocollo development-only congelato prima del primo output; baseline implementata: drums-only vs drums+bass kick-fusion + low-end pYIN; Basic Pitch 0.4.0 congelata come candidata separata ma non eseguibile finché manca venv/lock/model freeze; prossimo passo = preflight locale no-transcription sulle 8 family |
