...
Source Separation pilot **preparato**: preflight reale passato su 8/8 family development e run append-only `source-separation-pilot-v1-001` creato con SHA delle sorgenti verificati; inferenza non ancora avviata. Baseline primaria: `intel-openvino-htdemucs-v4-97fc578`. Doctor reale PASS: Audacity 3.7.1, `mod-openvino.dll`, `mod-script-pipe.dll` e i due hash HTDemucs congelati sono presenti. La pipe reale funziona, ma `GetInfo` non espone `OpenVINO Music Separation` come comando automatizzabile. Verifica sorgente Intel: modalità 2/4-stem, device e shifts sono stato GUI dell'effetto e non sono esposti da `VisitSettings`; il default interno è 2-stem. L'adapter batch via Audacity viene quindi chiuso come strada non idonea. Doctor standalone reale: Python 3.10.11 e FFmpeg 9.0.1 disponibili; Torch/OpenVINO non presenti nel Python globale. Fissato bootstrap dedicato coerente con le versioni della toolchain Intel del plugin (`torch 2.4.1+cpu`, `openvino 2024.6.0`) e script append-only che crea `D:\\FAME_NEURAL\\venv-source-separation`, verifica modello/runtime e produce `pip freeze --all` + receipt. Il freeze reale del venv dedicato è stato revisionato e congelato nel lock transitivo `requirements-source-separation-lock.txt` (16 package; source receipt SHA256 `e7fd1df0076b79101923900aa280b3c53a46c5b0a166bad75cbf972b7794411a`). Il lock è versionato e dispone di verifier read-only; nessuna inferenza è stata eseguita. Il prossimo gate è implementare e verificare il clean standalone HTDemucs/OpenVINO adapter sul solo development. Training serio resta chiuso.
...
| Source Separation pilot | **ENV STANDALONE LOCKED / ADAPTER DA IMPLEMENTARE** | run 8/8 development già preparato; venv dedicato verificato con `torch 2.4.1+cpu`, `openvino 2024.6.0`, modello congelato leggibile e FFmpeg disponibile; freeze transitivo 16 package revisionato e lock committato; nessuna inferenza eseguita; prossimo gate = clean standalone HTDemucs/OpenVINO adapter sul solo development |
