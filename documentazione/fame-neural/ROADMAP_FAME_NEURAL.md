...
Per ogni livello registrare input e riferimenti, fattore di complessità introdotto, metriche e criteri di avanzamento definiti prima dei risultati. Conservare i livelli precedenti come regressioni e riportare gli esiti anche per livello: una media complessiva non deve nascondere un failure semplice. L'easy sanity di P3 è il punto di partenza diagnostico; questo curriculum organizza anche lo sviluppo successivo.

**Eventuale curriculum di training:** solo se P4/P5 motivano un metodo apprendibile e vengono soddisfatti i requisiti per aprire il relativo training, predisporre un confronto tra progressione easy→hard e uso degli stessi dati a complessità mescolata, con split, modello e budget comparabili e criteri di scelta fissati prima. Il vantaggio del curriculum è un'ipotesi da verificare, non un risultato acquisito né un obbligo di passare a un metodo apprendibile. Training resta chiuso; questa previsione non lo autorizza. La selezione usa development/validation ammessi, mai i 12 consumati o il nuovo cohort indipendente P6.
...
- Confrontare fusion attiva/disattiva sullo stesso development, mantenendo uguali gli altri componenti. Il miglioramento di usefulness già osservato nel development resta evidenza storica; il nuovo confronto deve quantificare per evento kick corretti recuperati, falsi positivi introdotti e kick ancora mancanti, oltre a precision/recall complessive. Definire prima riferimento, matching uno-a-uno e tolleranza temporale, riportando conteggi e denominatori per family e livello del curriculum; senza annotazioni affidabili non dichiarare questi conteggi verificati. Non imporre regolarità ritmica che elimini sincopi reali.
...
- estendere la copertura metadata style/beat/split oltre il batch tecnico già chiuso in 7C Block2;
