"""Direct QA worker v3: fresh reconstruction repair without replaying the rejected candidate.

V2 remains frozen for historical desks. V3 preserves first-attempt measurement,
performs at most one semantic repair, never exposes tools/rubric/targets/check
IDs, and deliberately does NOT replay the rejected candidate into attempt 2.
"""
import argparse
import hashlib
import json
...
VERSION='fame-direct-qa-worker-v3'
MODEL=base.MODEL
DIGEST=base.DIGEST
...
def code_hashes():
    names=('direct_qa_worker_v3.py','direct_qa_worker.py','agent.py')
...
def metadata(p):
    return dict(
        ...
        modelDigest=DIGEST,
        options=OPTIONS,
        thinking='server default; not overridden',
        systemSha256=digest(SYSTEM),
        maximumModelCalls=MAXIMUM_MODEL_CALLS,
        retries=1,
        repairPolicy='FRESH_RECONSTRUCTION_GENERIC_VALIDATOR_FEEDBACK_NO_REPLAY_NO_CHECK_IDS_NO_EXPECTED_VALUES',
    )
...
def init(root,task):
    ...
    print('Scrivania v3 creata; nessuna chiamata al modello: '+str(root))


def verify(root):
    m=agent.read(agent.safe_path(root,'desk.json'))
    p=package(m['taskId'])
    if m!=metadata(p):raise ValueError('Protocollo o codice v3 cambiato dopo init')
...
def repair_payload(p,validation):
    req=payload(p)
    classes=feedback_classes(validation)
    feedback=(
        'Il primo tentativo non ha superato il validatore host. '
        'In questa chiamata non viene riproposta la risposta precedente e devi ricostruire '
        'l intera risposta indipendentemente dallo snapshot originale. '
        ...
        'Rileggi tutte le affermazioni e tutte le unita da zero. '
        'Per ogni conclusione ricontrolla esplicitamente negazioni, condizioni, quantificatori '
        'e differenze tra cio che la fonte afferma e cio che sarebbe solo un inferenza. '
        'Usa solo evidenze necessarie e restituisci esclusivamente il JSON richiesto.'
    )
    req['messages']=req['messages']+[dict(role='user',content=feedback)]
    return req,classes
...
def run(root,client=None):
    ...
    with lock.open('x') as f:f.write('Direct QA v3 attivo; rimuovere solo dopo arresto confermato.\n')
    try:
        p=verify(root)
        if agent.safe_path(root,'attempt-1').exists():
            raise FileExistsError('attempt-1 esiste gia; il task v3 non e rieseguibile')
        ...
        try:
            ...
            verify(root)
            print('QA diretto v3 attempt 1/2: attesa Ollama...',flush=True)
        ...
        started2=None
        try:
            req2,classes=repair_payload(p,first_validation)
            agent.write(out2/'repair-feedback.json',{
                'classes':classes,
            ...
            verify(root)
            print('QA diretto v3 attempt 2/2: ricostruzione fresca controllata...',flush=True)
            ...
            report2.update(
                status='VALIDATED_FOR_REVIEW_AFTER_REPAIR' if validation2['accepted'] else 'REJECTED',
                firstAttemptPass=False,
                acceptedAfterRepair=validation2['accepted'],
                validation=validation2,
                repairFeedbackClasses=classes,
                previousCandidateReplayed=False,
            )
...
def status(root):
    ...
    if r1['status']!='REJECTED_REPAIR_PENDING':raise ValueError('Stato attempt-1 v3 non riconosciuto')
    ...
    if r2['status'] in ('VALIDATED_FOR_REVIEW_AFTER_REPAIR','REJECTED'):
        ...
        expected_req2,classes=repair_payload(p,v1)
        if agent.read(out2/'request.json')!=expected_req2:raise ValueError('Request repair v3 non conforme')
        ...
        if r2.get('previousCandidateReplayed') is not False:raise ValueError('Replay candidate inatteso in v3')
    return r2
