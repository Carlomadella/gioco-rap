"""Prepare and grade a second frozen real FAME QA case for Cline; never invokes a model."""
import argparse
import hashlib
import json
...
VERSION = 'fame-cline-gptoss-tsumugi-qa-probe-v1'
MODEL = 'gpt-oss:20b'
CASES = ('MISSING_EVENTS', 'TIMBRE_HYPOTHESIS', 'ONSET_TOLERANCE_CAUSE', 'PAIR_CONFIDENCE_UNKNOWN')

TASK = '''# FAME Neural — QA reale Tsumugi ristretto
...
RULE = '''# Scrivania Cline GPT-OSS — Tsumugi QA
...
Cita solo evidenceIds presenti in evidence.json e usa il minimo insieme sufficiente.
Non dichiarare PASS del test: la valutazione appartiene al correttore esterno.
'''

PROCEDURE = '''# Procedura TSUMUGI-REAL-QA-1
Per ogni voce di TASK.json, nello stesso ordine:
1. Leggi l'affermazione completa e il report intero. evidence.json assegna un evidenceId a ogni riga non vuota.
...
3. Se anche una parte non è dimostrata, usa supported=false ed evidenceIds=[].
4. Distingui osservazioni, limiti e ipotesi. Non trasformare una possibilità in una causa provata.
5. Non usare righe solo vicine o di contesto se non servono alla copertura dell'affermazione.
Scrivi JSON senza Markdown, esattamente:
{"results":[{"code":"CODICE","supported":true,"evidenceIds":["E001"]}]}
Un risultato per voce nell'ordine di TASK.json, nessuna chiave aggiuntiva.
'''
...
def text_and_tasks():
    ...
    task = {
        'schema': VERSION,
        'cases': [{'code': code, 'assertion': qa.CATALOG[code]} for code in CASES],
    }
    return text, task


def desk_files():
    text, task = text_and_tasks()
    return {
        'TASK.md': TASK,
        ...
        'report.md': text,
        'evidence.json': encoded(qa.evidence_records(text)),
    }
...
def assess_answer(actual):
    text, _ = text_and_tasks()
    errors = []
    assessments = []
    if type(actual) is not dict or set(actual) != {'results'} or type(actual['results']) is not list:
        return False, ['OUTPUT_OBJECT_CONTRACT'], assessments
    rows = actual['results']
    if len(rows) != len(CASES):
        return False, ['RECORD_COUNT'], assessments

    for index, (code, row) in enumerate(zip(CASES, rows)):
        prefix = f'RECORD_{index}'
        if (type(row) is not dict or set(row) != {'code', 'supported', 'evidenceIds'}
                or row.get('code') != code or type(row.get('supported')) is not bool
                or type(row.get('evidenceIds')) is not list):
            errors.append(prefix + '_CONTRACT')
            continue

        ids = row['evidenceIds']
        if (len(ids) > 8 or any(type(e) is not str for e in ids)
                or len(set(ids)) != len(ids)):
            errors.append(prefix + '_EVIDENCE_IDS')
            continue

        expected_supported = code in qa.RUBRIC
        if row['supported'] != expected_supported:
            assessments.append({
                'code': code,
                'conclusion': 'INCORRECT',
                'evidence': 'NOT_ASSESSED',
                'surplusEvidenceIds': [],
                'status': 'INCORRECT_CONCLUSION',
            })
            errors.append(prefix + '_INCORRECT_CONCLUSION')
            continue

        if not expected_supported:
            if ids:
                errors.append(prefix + '_UNSUPPORTED_REQUIRES_EMPTY_EVIDENCE')
                continue
            assessments.append({
                'code': code,
                'conclusion': 'CORRECT',
        ...
        finding = {'code': code, 'evidenceIds': ids}
        finding_errors = qa.validate_finding(finding, text)
        if finding_errors:
            assessments.append({
                'code': code,
                'conclusion': 'CORRECT',
                ...
                'rubricErrors': finding_errors,
            })
            errors.append(prefix + '_INSUFFICIENT_EVIDENCE')
            continue

        drops = qa.evidence_drops({'findings': [finding]}, text)
        surplus = drops[0]['droppedEvidenceIds'] if drops else []
        if surplus:
            assessments.append({
                'code': code,
                'conclusion': 'CORRECT',
                'evidence': 'DIRECT',
                'surplusEvidenceIds': surplus,
                'status': 'SURPLUS_EVIDENCE',
            })
    ...
    return not errors and len(assessments) == len(CASES), errors, assessments
...
def grade(root):
    ...
    result = {
        ...
        'limits': ('Restricted reuse of the frozen Tsumugi controlled QA report. '
                   'A pass does not establish generalization, production readiness, '
                   'or independently verify Cline logs/model isolation.'),
    }
