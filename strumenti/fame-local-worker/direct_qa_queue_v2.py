"""Sequential queue for direct QA worker v2 with one controlled repair per desk."""
import argparse
import json
from pathlib import Path
import agent
import direct_qa_worker_v2 as worker
...
def init(root,tasks):
    ...
    agent.write(root/'queue.json',dict(schema=SCHEMA,tasks=tasks,
        ...
        executionAuthorized=False,maximumModelCallsPerTask=2))
...
def state(root):
    ...
    if config.get('schema')!=SCHEMA or not tasks or len(tasks)>20 or len(set(tasks))!=len(tasks):
        raise ValueError('Coda v2 invalida')
...
def run(root,client=None):
    ...
    with lock.open('x') as f:f.write('Coda QA v2 attiva\n')
    try:
        current=state(root)
