...
$receiptRunner = Join-Path $here "basic-pitch-development-inference-v3.js"
$executor = Join-Path $here "basic-pitch-development-execute-v3.py"
...
foreach ($file in @($lockVerifier, $receiptRunner, $executor, $python)) {
  if (-not (Test-Path -LiteralPath $file -PathType Leaf)) {
    throw "Basic Pitch v1-003 execution prerequisite mancante: $file"
  }
}
...
Write-Host "2/4 Verify frozen v1-003 AUTHORIZED_NO_INFERENCE receipt..." -ForegroundColor Cyan
& node $receiptRunner check $Workspace $RunId
if ($LASTEXITCODE -ne 0) { throw "Basic Pitch v1-003 receipt check failed" }

Write-Host "3/4 Run v1-003 executor self-test (NO audio / NO inference)..." -ForegroundColor Cyan
& $python $executor self-test
if ($LASTEXITCODE -ne 0) { throw "Basic Pitch v1-003 executor self-test failed" }

Write-Host "4/4 Execute Basic Pitch v1-003 on locked 8-family development bass cohort..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "Basic Pitch v1-003 development inference failed"
}
