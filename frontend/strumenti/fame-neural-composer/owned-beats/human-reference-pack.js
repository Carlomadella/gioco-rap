...
h+='<div class="card"><h3>Beat reference</h3><div class="row"><div><label>BPM reference</label><input id="bpm" type="number" min="20" max="400" step="0.001" value="'+(f.beatReference.referenceBpm==null?'':f.beatReference.referenceBpm)+'"></div><div><label>Metric level</label><select id="metricLevel">'+opts(levels,f.beatReference.metricLevel)+'</select></div><button class="action" id="calcBpm">Calcola BPM dai tap</button><label><input type="checkbox" id="beatReviewed" '+(f.beatReference.reviewed?'checked':'')+'> beat reference revisionata</label></div><div id="windows"></div></div>';
...
var wr=document.getElementById('windows');wr.innerHTML=f.beatReference.windows.map(function(w,i){return '<div class="card"><b>'+esc(w.position)+'</b> · assoluto '+w.startSeconds+'s → '+(w.startSeconds+w.durationSeconds).toFixed(3)+'s<audio controls id="wa'+i+'" src="'+esc(w.audioPath)+'"></audio><div class="row"><button class="action tap" data-w="'+i+'">Registra beat</button><button class="action clear" data-w="'+i+'">Azzera beat</button><label><input class="wreview" data-w="'+i+'" type="checkbox" '+(w.reviewed?'checked':'')+'> finestra revisionata</label></div><div class="beats" id="beats'+i+'">'+(w.beatTimesSeconds.join(', ')||'—')+'</div></div>'}).join('');
...
wr.querySelectorAll('.tap').forEach(function(b){b.onclick=function(){var i=Number(b.dataset.w),a=document.getElementById('wa'+i),t=Number(a.currentTime.toFixed(6)),w=f.beatReference.windows[i];if(t>=0&&t<=w.durationSeconds&&!w.beatTimesSeconds.includes(t)){w.beatTimesSeconds.push(t);w.beatTimesSeconds.sort(function(a,b){return a-b});save();document.getElementById('beats'+i).textContent=w.beatTimesSeconds.join(', ')||'—'}}});wr.querySelectorAll('.clear').forEach(function(b){b.onclick=function(){var i=Number(b.dataset.w),w=f.beatReference.windows[i];w.beatTimesSeconds=[];save();document.getElementById('beats'+i).textContent='—'}});wr.querySelectorAll('.wreview').forEach(function(x){x.onchange=function(){f.beatReference.windows[Number(x.dataset.w)].reviewed=x.checked;save()}});
    ...
    try {
      ...
      for (const record of records) {
        ...
        for (const window of windows) {
          if (window.position === "FULL_TRACK") continue;
          const source = path.join(workspace, record.localPath);
