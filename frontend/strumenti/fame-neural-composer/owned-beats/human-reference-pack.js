...
function validateSubmissionCore(submission, manifest, protocolInfo, expectedRecords, durationsById) {
  ...
  for (const family of submission.families) {
    ...
    windows.forEach((window, index) => {
      ...
      const coverage = window.coverage || "PENDING";
      if (!["PENDING", "COMPLETE", "NO_BEAT", "AMBIGUOUS"].includes(coverage)) {
        throw new Error(`Invalid coverage: ${record.sourceRecordId}:${expected.position}`);
      }
      const notes = typeof window.coverageNotes === "string" ? window.coverageNotes.trim() : "";
      const referenceBpm = Number(family.beatReference?.referenceBpm);
      // Density is a review warning, not a universal rhythmic quality threshold.
      // Sparse musical passages require an explicit explanation, never fabricated beats.
      const sparse = beats.length < expected.durationSeconds * referenceBpm / 60 * 0.5;
      ...
      if (!complete && !noBeat) windowsUsable = false;
    });
...
let current=0,timerStarted=null,timerFamily=null,selectedBeat=null,activeWindow=null,playheadRaf=null;
function invalidateBeat(wi){var f=state.families[current];f.beatReference.reviewed=false;if(wi!=null){f.beatReference.windows[wi].reviewed=false;var box=document.querySelector('.wreview[data-w="'+wi+'"]');if(box)box.checked=false}var all=document.getElementById('beatReviewed');if(all)all.checked=false}
function stopReviewTimer(){if(timerStarted!==null&&timerFamily!==null){var f=state.families[timerFamily];f.reviewCostSeconds=Number((Number(f.reviewCostSeconds||0)+(performance.now()-timerStarted)/1000).toFixed(3));timerStarted=null;timerFamily=null;save()}}
