...
const CONSUMED_HOLDOUT_REFERENCE_FILE=path.join(HERE,"audio-analysis-holdout-cohort-r1-v2.json");
...
function validateProtocol(){
  ...
  if(
    p.schema!=="fame-owned-beats-audio-to-midi-independent-evaluation-protocol-v1"||
    p.version!==1||
    p.status!=="FROZEN_BEFORE_FRESH_COHORT_SELECTION"||
    p.sourceCorpus?.expectedRecords!==131||
    p.sourceCorpus?.expectedCompositionFamilies!==131||
    p.cohort?.cohortId!==COHORT_ID||
    p.cohort?.plannedSplit!=="audio-to-midi-evaluation-v1"||
    ...
    p.frozenPipeline?.drums?.armId!=="drums-bass-kick-fusion-v1"||
    p.frozenPipeline?.lowEnd?.armId!=="librosa-pyin-lowend-v1"||
    p.frozenPipeline?.basicPitch?.mayReenterThisEvaluation!==false||
...
function validateGlobalIdentityIntegrity(manifest,p){
  ...
  const seen={compositionFamilyId:new Map(),sourceRecordId:new Map(),sourceAssetId:new Map(),sha256:new Map()};
  for(const record of manifest.records){
    const id=identity(record);
    for(const field of Object.keys(seen)){
      if(seen[field].has(id[field]))throw new Error("Duplicate manifest "+field+": "+id[field]);
      seen[field].set(id[field],id.sourceRecordId);
    }
  }
...
function untouchedForEvaluation(record){
...
function priorExcludedIdentities(){
  ...
  const consumed=readJson(CONSUMED_HOLDOUT_REFERENCE_FILE);
  const sourceIds=new Set(dev.sourceSeparation?.expectedSourceRecordIds||[]);
  const familyIds=new Set();
  for(const row of consumed.records||[]){
    sourceIds.add(row.sourceRecordId);
    familyIds.add(row.compositionFamilyId);
  }
  return{sourceIds,familyIds};
}
function rankFor(record,seed){
  const id=identity(record);
  const canonical=IDENTITY_FIELDS.map(k=>id[k]).join("|");
  return sha256Text(seed+"|"+canonical);
}
function select(manifest,p){
  ...
  const eligible=manifest.records.filter(record=>{
    const id=identity(record);
    return untouchedForEvaluation(record)&&
      !excluded.sourceIds.has(id.sourceRecordId)&&
      !excluded.familyIds.has(id.compositionFamilyId);
  });
  const expected=p.cohort.expectedFamilies;
  if(eligible.length<expected)throw new Error("Insufficient fresh untouched families: "+eligible.length+" < "+expected);
  const seed=p.cohort.selection.seed;
  const ranked=eligible.map(record=>({record,rankSha256:rankFor(record,seed)}))
    .sort((a,b)=>a.rankSha256.localeCompare(b.rankSha256)||a.record.sourceRecordId.localeCompare(b.record.sourceRecordId));
  const chosen=ranked.slice(0,expected).map((item,index)=>({
    ...identity(item.record),
    selectionRank:index+1,
    rankSha256:item.rankSha256
  }));
  return{
    eligible,
    chosen,
...
function referenceFrom(manifest,p,selection){
  return{
    schema:"fame-owned-beats-audio-to-midi-evaluation-cohort-reference-v1",
    version:1,
    cohortId:p.cohort.cohortId,
    status:"SELECTED_METADATA_ONLY_AWAITING_REPO_FREEZE",
    purpose:p.purpose,
    createdAt:new Date().toISOString(),
    expectedFamilies:p.cohort.expectedFamilies,
    plannedSplit:p.cohort.plannedSplit,
    identityFields:IDENTITY_FIELDS,
    selection:{
      algorithm:p.cohort.selection.algorithm,
      seed:p.cohort.selection.seed,
    ...
    safety:{
      ...
      derivedAudioMetricsReadByThisCommand:false,
...
function outputRoot(workspace){
  return path.join(workspace,"runs","audio-to-midi-independent-evaluation-preparation",COHORT_ID);
}
function prepare(workspaceRoot){
  const workspace=path.resolve(workspaceRoot),manifestFile=path.join(workspace,"manifest","owned-beats-manifest.json");
  if(!fs.existsSync(manifestFile))throw new Error("Owned-beats manifest missing: "+manifestFile);
  const p=validateProtocol(),manifest=readJson(manifestFile),selection=select(manifest,p),reference=referenceFrom(manifest,p,selection);
  const root=outputRoot(workspace),file=path.join(root,"cohort-reference.json");
  if(fs.existsSync(file)){
    const existing=readJson(file);
    if(
      existing.schema!==reference.schema||
      existing.cohortId!==reference.cohortId||
      existing.cohortDigestSha256!==reference.cohortDigestSha256||
      JSON.stringify(existing.records)!==JSON.stringify(reference.records)
    ) throw new Error("Existing evaluation cohort preparation differs from deterministic selection");
    return result(existing,file,true);
  }
  fs.mkdirSync(root,{recursive:true});
  fs.writeFileSync(file,stableJson(reference),{flag:"wx"});
  return result(reference,file,false);
}
function preview(workspaceRoot){
  const workspace=path.resolve(workspaceRoot),manifestFile=path.join(workspace,"manifest","owned-beats-manifest.json");
  if(!fs.existsSync(manifestFile))throw new Error("Owned-beats manifest missing: "+manifestFile);
  const p=validateProtocol(),manifest=readJson(manifestFile),selection=select(manifest,p),reference=referenceFrom(manifest,p,selection);
  return result(reference,null,false);
}
function result(reference,file,alreadyPrepared){
  return{
    mode:file?"AUDIO_TO_MIDI_INDEPENDENT_EVALUATION_COHORT_PREPARED":"AUDIO_TO_MIDI_INDEPENDENT_EVALUATION_COHORT_PREVIEW",
    cohortId:reference.cohortId,
    status:reference.status,
    expectedFamilies:reference.expectedFamilies,
    eligibleFamilyCount:reference.selection.eligibleFamilyCount,
    ...
    selectedSourceRecordIds:reference.records.map(x=>x.sourceRecordId),
    alreadyPrepared,
    outputFile:file,
    audioFileOpenedByThisCommand:false,
    audioFileHashedByThisCommand:false,
    ...
    trainingAuthorized:false,
    taskDataReadyMayBeDeclared:false,
    nextAction:file?"FREEZE_COHORT_REFERENCE_IN_REPOSITORY_BEFORE_ANY_AUDIO_ACCESS":"PREPARE_APPEND_ONLY_COHORT_REFERENCE"
  };
}
function report(workspaceRoot){
  const file=path.join(outputRoot(path.resolve(workspaceRoot)),"cohort-reference.json");
  if(!fs.existsSync(file))throw new Error("Prepared evaluation cohort reference missing");
...
function selfTest(){
  ...
  const fixture=[];
  for(let i=1;i<=131;i++){
    const rid="FAME"+String(i).padStart(6,"0");
    const sha=crypto.createHash("sha256").update("fixture-"+rid).digest("hex");
    fixture.push({
  ...
  for(const rid of p.sourceCorpus?[]:[])void rid;
  const dev=readJson(DEVELOPMENT_PROTOCOL_FILE).sourceSeparation.expectedSourceRecordIds;
  const hold=(readJson(CONSUMED_HOLDOUT_REFERENCE_FILE).records||[]).map(x=>x.sourceRecordId);
  for(const rid of [...dev,...hold]){
    const row=fixture.find(x=>x.sourceRecordId===rid);
    if(row)row.split="already-used";
  }
  const manifest={schema:"fame-owned-beats-workspace-v1",version:1,records:fixture};
  const a=select(manifest,p),b=select(manifest,p);
  if(a.chosen.length!==12||a.eligible.length!==113)throw new Error("Fresh cohort fixture counts failed");
  if(a.cohortDigestSha256!==b.cohortDigestSha256||JSON.stringify(a.chosen)!==JSON.stringify(b.chosen))throw new Error("Fresh cohort selection not deterministic");
  return{
    mode:"AUDIO_TO_MIDI_INDEPENDENT_EVALUATION_SELECTOR_SELF_TEST_PASS",
    eligibleFamilies:113,
    selectedFamilies:12,
    audioFileOpenedByThisCommand:false,
    audioFileHashedByThisCommand:false
  };
...
function main(args=process.argv.slice(2)){
  ...
  if(command==="self-test")out=selfTest();
    ...
    if(!workspace)throw new Error("Usage: node prepare-audio-to-midi-independent-evaluation.js <preview|prepare|report> <workspace>");
    if(command==="preview")out=preview(workspace);
    else if(command==="prepare")out=prepare(workspace);
    else if(command==="report")out=report(workspace);
    else throw new Error("Unknown command");
  }
  process.stdout.write(stableJson(out));
}
if(require.main===module){try{main()}catch(error){console.error("AUDIO TO MIDI INDEPENDENT EVALUATION PREPARATION FAILED: "+error.message);process.exitCode=1}}
module.exports={identity,identityDigest,untouchedForEvaluation,rankFor,select,preview,prepare,report,selfTest};
