...
EXECUTION_CONTRACT_FILE = HERE / "basic-pitch-execution-contract-v1.json"
IMPLEMENTATION_CONTRACT_FILE = HERE / "basic-pitch-execution-implementation-v1.json"
LOCK_FILE = HERE / "requirements-basic-pitch-lock.txt"

DEFAULT_RUN_ID = "basic-pitch-development-inference-v1-001"
RECEIPT_SCHEMA = "fame-owned-beats-basic-pitch-development-inference-v1"
RESULT_SCHEMA = "fame-owned-beats-basic-pitch-development-inference-result-v1"
SUMMARY_SCHEMA = "fame-owned-beats-basic-pitch-development-inference-summary-v1"
...
def validate_execution_contract(contract):
    if (
        contract.get("schema") != "fame-owned-beats-basic-pitch-execution-contract-v1"
        or contract.get("version") != 1
        or contract.get("status") != "FROZEN_BEFORE_FIRST_BASIC_PITCH_OUTPUT"
...
def validate_implementation_contract(contract):
    if (
        contract.get("schema") != "fame-owned-beats-basic-pitch-execution-implementation-v1"
        or contract.get("version") != 1
        or contract.get("status") != "FROZEN_BEFORE_FIRST_BASIC_PITCH_INFERENCE"
...
def execute(workspace_root: str, run_id: str = DEFAULT_RUN_ID):
    ...
    if summary_file.exists():
        ...
        return {
            "mode": "BASIC_PITCH_DEVELOPMENT_INFERENCE_ALREADY_COMPLETE",
            "runId": run_id,
            "records": 8,
    ...
    return {
        "mode": "BASIC_PITCH_DEVELOPMENT_INFERENCE_COMPLETE",
        "runId": run_id,
        "records": 8,
        "status": summary["status"],
        "modelLoadedOncePerProcess": True,
...
def self_test():
    ...
    return {
        "mode": "BASIC_PITCH_DEVELOPMENT_EXECUTOR_SELF_TEST_PASS",
