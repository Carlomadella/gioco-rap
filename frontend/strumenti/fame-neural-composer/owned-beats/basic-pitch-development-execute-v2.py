...
EXECUTION_CONTRACT_FILE = HERE / "basic-pitch-execution-contract-v2.json"
IMPLEMENTATION_CONTRACT_FILE = HERE / "basic-pitch-execution-implementation-v2.json"
LOCK_FILE = HERE / "requirements-basic-pitch-lock.txt"

DEFAULT_RUN_ID = "basic-pitch-development-inference-v1-002"
RECEIPT_SCHEMA = "fame-owned-beats-basic-pitch-development-inference-v2"
RESULT_SCHEMA = "fame-owned-beats-basic-pitch-development-inference-result-v2"
SUMMARY_SCHEMA = "fame-owned-beats-basic-pitch-development-inference-summary-v2"
...
def validate_execution_contract(contract):
    if (
        contract.get("schema") != "fame-owned-beats-basic-pitch-execution-contract-v2"
        or contract.get("version") != 2
        or contract.get("status") != "FROZEN_BEFORE_FIRST_V2_BASIC_PITCH_OUTPUT"
        ...
        or contract.get("supersedes", {}).get("runId") != "basic-pitch-development-inference-v1-001"
...
def validate_implementation_contract(contract):
    if (
        contract.get("schema") != "fame-owned-beats-basic-pitch-execution-implementation-v2"
        or contract.get("version") != 2
        or contract.get("status") != "FROZEN_BEFORE_FIRST_V2_BASIC_PITCH_INFERENCE"
...
def validate_receipt(workspace: Path, run_id: str, frozen):
    ...
    if (
        ...
        or receipt.get("evidence", {}).get("preInferenceGatePassedImmediatelyBeforeReceipt")
        is not True
        or receipt.get("evidence", {}).get("audioDecodedByPrepareCommand") is not False
        or receipt.get("evidence", {}).get("basicPitchInferenceExecutedByPrepareCommand")
        is not False
        or receipt.get("evidence", {}).get("midiWrittenByPrepareCommand") is not False
    ):
...
def execute(workspace_root: str, run_id: str = DEFAULT_RUN_ID):
    ...
    if summary_file.exists():
        ...
        return {
            "mode": "BASIC_PITCH_SUPERSEDING_DEVELOPMENT_INFERENCE_ALREADY_COMPLETE",
            "runId": run_id,
            "records": 8,
    ...
    return {
        "mode": "BASIC_PITCH_SUPERSEDING_DEVELOPMENT_INFERENCE_COMPLETE",
        "runId": run_id,
        "records": 8,
        "status": summary["status"],
        "modelLoadedOncePerProcess": True,
...
def self_test():
    ...
    return {
        "mode": "BASIC_PITCH_SUPERSEDING_EXECUTOR_SELF_TEST_PASS",
