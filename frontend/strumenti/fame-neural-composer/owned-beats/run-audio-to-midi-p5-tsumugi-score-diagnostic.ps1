...
$diagProtocol = Join-Path $here "audio-to-midi-p5-tsumugi-score-diagnostic-v1.json"
$runner = Join-Path $here "audio-to-midi-p5-tsumugi-score-diagnostic.py"
$staticTest = Join-Path $composer "owned-beats-audio-to-midi-p5-tsumugi-score-diagnostic-test.py"

foreach ($file in @($envSpecPath,$diagProtocol,$runner,$staticTest)) {
  if (-not (Test-Path -LiteralPath $file -PathType Leaf)) {
    throw "Tsumugi score diagnostic dependency missing: $file"
  }
}
...
Write-Host "1/2 Tsumugi score diagnostic static/self-test..." -ForegroundColor Cyan
& $audioAnalysisPython $staticTest
if ($LASTEXITCODE -ne 0) {
  throw "Tsumugi score diagnostic static test failed"
}
& $venvPython $runner self-test
if ($LASTEXITCODE -ne 0) {
  throw "Tsumugi score diagnostic self-test failed"
}

Write-Host "2/2 Tsumugi pair-gate + Semi-CRF score diagnostic on frozen synthetic fixtures..." -ForegroundColor Cyan
& $venvPython $runner execute $Workspace
if ($LASTEXITCODE -ne 0) {
  throw "Tsumugi score diagnostic failed"
}
