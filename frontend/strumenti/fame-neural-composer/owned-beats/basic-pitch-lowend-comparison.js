...
const PROTOCOL_FILE=path.join(HERE,"basic-pitch-lowend-comparison-v1.json");
const QA_FILE=path.join(HERE,"basic-pitch-development-qa.py");

const REVIEW_ID="basic-pitch-lowend-blind-comparison-v1-001";
...
const PRIOR_REVIEW_ID="audio-to-midi-human-review-v1-001";

const PACKAGE_SCHEMA="fame-owned-beats-basic-pitch-lowend-comparison-package-v1";
const SUBMISSION_SCHEMA="fame-owned-beats-basic-pitch-lowend-comparison-submission-v1";
const REPORT_SCHEMA="fame-owned-beats-basic-pitch-lowend-comparison-report-v1";
const BLIND_KEY_SCHEMA="fame-owned-beats-basic-pitch-lowend-comparison-blind-key-v1";
...
function protocol(){
 ...
 if(
   p.schema!=="fame-owned-beats-basic-pitch-lowend-comparison-v1"||
   p.version!==1||
   p.status!=="FROZEN_BEFORE_FIRST_LOW_END_COMPARISON_LISTENING"||
...
function validatePriorHumanReview(workspace,p){
 const file=path.join(workspace,"reviews","audio-to-midi-development",PRIOR_REVIEW_ID,"report.json");
...
function renderPyin(result,p,sr=p.renderer.sampleRate){
 ...
 const end=Math.max(1,...low.notes.map(n=>n.endSeconds+.1)),samples=new Float32Array(Math.ceil(end*sr));
...
function renderBasicPitch(result,p,sr=p.renderer.sampleRate){
 const events=result.noteEvents;
 if(!Array.isArray(events))throw new Error("Basic Pitch note events invalid");
 const end=Math.max(1,...events.map(n=>n.endSeconds+.1)),samples=new Float32Array(Math.ceil(end*sr));
...
document.getElementById('submit').onclick=async()=>{const families=PKG.families.map(f=>({sourceRecordId:f.sourceRecordId,...state(f.sourceRecordId)}));const body={schema:'fame-owned-beats-basic-pitch-lowend-comparison-submission-v1',version:1,reviewId:PKG.reviewId,packageDigestSha256:PKG.packageDigestSha256,families};const r=await fetch('/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const t=await r.text();document.getElementById('result').textContent=t;if(!r.ok)alert(t)};
...
function prepare(workspaceRoot,reviewId=REVIEW_ID){
 ...
 const pkg={
   ...
   families:ids.map(sourceRecordId=>({sourceRecordId,candidates:["A","B"],referenceStem:"bass"})),
   safety:p.safety
 };
...
function serve(workspaceRoot,reviewId=REVIEW_ID,port=0){
 ...
 const server=http.createServer((req,res)=>{try{
   ...
   if((req.method==="GET"||req.method==="HEAD")&&parts[0]==="render"&&parts.length===3){
     ...
     if(!wav){wav=armId==="librosa-pyin-lowend-v1"?renderPyin(pyinResult(workspace,rid),p):renderBasicPitch(bpResult(workspace,rid),p);cache.set(cacheKey,wav)}
...
module.exports={prepare,median,computeArmStats,chooseLowEnd,validateSubmission,finalizeSubmission,renderPyin,renderBasicPitch};
