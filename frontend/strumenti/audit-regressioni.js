...
console.log("\nProblemi riscontrati — in carcere niente notifiche, e le didascalie sono nostre");
...
test("le didascalie della strada non citano nessuna lirica vera",
  !/\bby:\s*"/.test(caption) && !/"by"\s*:/.test(caption) && !caption.includes("track") &&
  !/Wu-Tang|2Pac|Tupac|Notorious|Jay-Z|Eminem|Kendrick|Drake|Snoop|Mobb Deep|\bNas\b/.test(caption));
test("ogni didascalia dice da dove arriva, non chi l'ha scritta",
...
test("e sono abbastanza da reggere il filtro delle ultime 28 uscite",
  ...
  caption.includes("recent.slice(-28)"));
